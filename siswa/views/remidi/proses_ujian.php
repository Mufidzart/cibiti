              ...
              if ($getkunci->num_rows !== 0) : ?>
                <?php while ($kunci = mysqli_fetch_assoc($getkunci)) :
                  ...
                  <div class="form-check form-check-custom form-check-solid p-2">
                    <input class="form-check-input radio_jawaban" type="radio" value="<?= $kunci['jawaban'] ?>" name="jawaban_<?= $id_soal ?>" data-id-penugasan="<?= $idpenugasan ?>" data-kode="<?= $kode_tugas ?>" data-id-soal="<?= $id_soal ?>" data-id-jawaban="<?= $kunci['id'] ?>" <?= $selected ?>>
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('.radio_jawaban').on('click', function() {
      ...
      var jenis_ujian = "<?= $jenis_ujian ?>";
      $.ajax({
        url: 'backend/function.php?action=push_jawaban_remidi',
        type: 'post',
        data: {
          jawaban: jawaban,
          id_penugasan: id_penugasan,
